...
export class ApiService {
  ...
  getApi(category: string, title: string): Observable<PartialAnimeManga[]> {
    ...
    return this.http
      .get<{ results: IPartialAnimeManga[] }>(
        `${environment.searchUrlApi}${type}`,
        { params }
      )
      .pipe(
        map((response) =>
          response.results.map((item) => PartialAnimeManga.Build(item))
        )
      );
  }
  ...
  getTopTitle(category: string): Observable<PartialContent[]> {
    return this.http
      .get<{ top: IPartialContent[] }>(`${environment.topApi}${category}/1`)
      .pipe(
        map((response) =>
          response.top.map((item) => PartialContent.Build(item))
        )
      );
  }
}
